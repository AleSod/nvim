...
local function getJdtlsConfigLocation()
  if vim.fn.has 'mac' > 0 then
    return home .. '/.local/share/nvim/mason/packages/jdtls/config_mac'
  elseif vim.fn.has 'linux' > 0 then
    return home .. '/.local/share/nvim/mason/packages/jdtls/config_linux'
  else
    print 'Unsupported/Unknown OS'
    return 'Unsupported'
  end
end
...
local config = {
  cmd = {
    ...
    '-javaagent:' .. home .. '/.local/share/nvim/mason/packages/jdtls/lombok.jar', -- NOTE: Must be installed manually!
    '-jar',
    vim.fn.glob(home .. '/.local/share/nvim/mason/packages/jdtls/plugins/org.eclipse.equinox.launcher.jar'),
    '-configuration',
    getJdtlsConfigLocation(),
    '-data',
    workspace_dir,
  },
